...
int n;
vector<int> piler_heights;
map<pair<int,int>, int> dp;  // Changed: map instead of array

int solve(int pilar_num, int diff){
    if(pilar_num == n){
        return (diff == 0) ? 0 : INT_MIN;
    }
    
    if(dp.count({pilar_num, diff}))  // Changed: map lookup
        return dp[{pilar_num, diff}];
    
    // Option 1: Add to pillar 1 (increases diff)
    int op1 = INT_MIN;
    int next1 = solve(pilar_num + 1, diff + piler_heights[pilar_num]);
    if(next1 != INT_MIN)
         op1 = next1 + piler_heights[pilar_num];
    
    // Option 2: Add to pillar 2 (decreases diff)
    int op2 = solve(pilar_num + 1, diff - piler_heights[pilar_num]);
    
    // Option 3: Skip current pole
    int op3 = solve(pilar_num + 1, diff);
    
    return dp[{pilar_num, diff}] = max({op1, op2, op3});  // Changed: map assignment
}
